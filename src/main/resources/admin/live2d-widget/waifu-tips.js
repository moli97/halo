...
function loadWidget(waifuPath, apiPath) {
	...
	function registerEventListener() {
		...
		/** 注册拖拽，目前不能往下拖，，， */
		document.querySelector("#waifu").addEventListener("mousedown", (e)=>{
			var positionDiv = $("#waifu").offset();
			...
			$(document).mousemove(function(e){
				...
				if(x<0){
					x=0;
				}else if(x>$(document).width()-$("#waifu").outerWidth(true)){
					x = $(document).width()-$("#waifu").outerWidth(true);
				}
				if(y<50){
					y=50;
				}else if(y>$(document).height()-$("#live2d").outerHeight(true)){
					y = $(document).height()-$("#live2d").outerHeight(true);
				}
				$("#waifu").css({
					'left':x+'px',
					'top':y+'px'
